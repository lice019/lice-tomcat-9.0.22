...
/**
 * 启动/关闭针对Catalina的shell程序。下面的命令行
 * <p>
 * 选项是公认的:
 * < ul >
 * 设置要处理的配置文件的路径名。如果指定了相对路径，它将被解释为相对于由“catalina”指定的目录路径名。基地”系统属性。李(conf / server.xml) < / >
 * -help -显示使用信息
 * - nonming -禁用命名支持
 * configtest -尝试测试配置
 * -启动一个Catalina实例
 ...
 */
//Catalina：实际上是tomcat项目的启动类，这个类被通过Bootstrap类来执行Catalina
public class Catalina {
    ...
    //tomcat中config目录下的server.xml配置文件
